...
export const ContentCard = ({ item, currentLanguage = "es" }: ContentCardProps) => {
  ...
  const TypeIcon = () => {
    switch (type) {
      case "article":
        return <FileText className="h-5 w-5 text-club-orange" aria-hidden="true" />;
      case "video":
        return <Video className="h-5 w-5 text-club-orange" aria-hidden="true" />;
      case "guide":
        return <BookOpen className="h-5 w-5 text-club-orange" aria-hidden="true" />;
      case "resource":
        return <Newspaper className="h-5 w-5 text-club-orange" aria-hidden="true" />;
      default:
        return <FileText className="h-5 w-5 text-club-orange" aria-hidden="true" />;
    }
  };
        ...
        <div className="h-48 overflow-hidden relative group">
          ...
          {type === "video" && (
            <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <Video className="h-12 w-12 text-white" aria-hidden="true" />
            </div>
          )}
        </div>
        <CardHeader>
          <div className="flex items-center justify-between mb-2">
            ...
            <time 
              ...
              className="text-xs text-club-brown/70 flex items-center gap-1"
            >
